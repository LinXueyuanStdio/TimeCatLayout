...
public class OpacityBar extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (isInEditMode()) {
            this.shader = new LinearGradient((float) this.mBarPointerHaloRadius, 0.0f, (float) x1, (float) y1, new int[]{8519424, -8257792}, null, TileMode.CLAMP);
            Color.colorToHSV(-8257792, this.mHSVColor);
        } else {
            this.shader = new LinearGradient((float) this.mBarPointerHaloRadius, 0.0f, (float) x1, (float) y1, new int[]{Color.HSVToColor(0, this.mHSVColor), Color.HSVToColor(255, this.mHSVColor)}, null, TileMode.CLAMP);
        }
        this.mBarPaint.setShader(this.shader);
        this.mPosToOpacFactor = 255.0f / ((float) this.mBarLength);
        this.mOpacToPosFactor = ((float) this.mBarLength) / 255.0f;
    ...
    public void setColor(int color) {
        ...
        this.shader = new LinearGradient((float) this.mBarPointerHaloRadius, 0.0f, (float) x1, (float) y1, new int[]{Color.HSVToColor(0, this.mHSVColor), color}, null, TileMode.CLAMP);
        this.mBarPaint.setShader(this.shader);
    ...
    public void setOpacity(int opacity) {
        this.mBarPointerPosition = Math.round(this.mOpacToPosFactor * ((float) opacity)) + this.mBarPointerHaloRadius;
        calculateColor(this.mBarPointerPosition);
        this.mBarPointerPaint.setColor(this.mColor);
        if (this.mPicker != null) {
            this.mPicker.setNewCenterColor(this.mColor);
    ...
    public int getOpacity() {
        int opacity = Math.round(this.mPosToOpacFactor * ((float) (this.mBarPointerPosition - this.mBarPointerHaloRadius)));
        if (opacity < 5) {
            return 0;
        }
        if (opacity > 250) {
    ...
    private void calculateColor(int coord) {
        ...
        this.mColor = Color.HSVToColor(Math.round(this.mPosToOpacFactor * ((float) coord)), this.mHSVColor);
