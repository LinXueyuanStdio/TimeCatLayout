...
public class SaturationBar extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (isInEditMode()) {
            this.shader = new LinearGradient((float) this.mBarPointerHaloRadius, 0.0f, (float) x1, (float) y1, new int[]{-1, -8257792}, null, TileMode.CLAMP);
            Color.colorToHSV(-8257792, this.mHSVColor);
        } else {
            this.shader = new LinearGradient((float) this.mBarPointerHaloRadius, 0.0f, (float) x1, (float) y1, new int[]{-1, Color.HSVToColor(255, this.mHSVColor)}, null, TileMode.CLAMP);
        }
        this.mBarPaint.setShader(this.shader);
        this.mPosToSatFactor = 1.0f / ((float) this.mBarLength);
        this.mSatToPosFactor = ((float) this.mBarLength) / 1.0f;
        ...
        if (isInEditMode()) {
            ...
            this.mBarPointerPosition = Math.round((this.mSatToPosFactor * hsvColor[1]) + ((float) this.mBarPointerHaloRadius));
        }
    }
    ...
    public void setColor(int color) {
        ...
        this.shader = new LinearGradient((float) this.mBarPointerHaloRadius, 0.0f, (float) x1, (float) y1, new int[]{-1, color}, null, TileMode.CLAMP);
        this.mBarPaint.setShader(this.shader);
    ...
    public void setSaturation(float saturation) {
        this.mBarPointerPosition = Math.round(this.mSatToPosFactor * saturation) + this.mBarPointerHaloRadius;
        calculateColor(this.mBarPointerPosition);
        this.mBarPointerPaint.setColor(this.mColor);
        if (this.mPicker != null) {
            this.mPicker.setNewCenterColor(this.mColor);
    ...
    private void calculateColor(int coord) {
        ...
        this.mColor = Color.HSVToColor(new float[]{this.mHSVColor[0], this.mPosToSatFactor * ((float) coord), 1.0f});
    }
