...
```
    */
    ...
    private void init(AttributeSet attrs) {
        ...
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.SmoothCheckBox_stroke_width, ViewUtil.dp2px(getContext(), 0));
        ta.recycle();
    ...
    public void setChecked(boolean checked) {
        setInitCheck(checked);
        if (mListener != null) {
            mListener.onCheckedChanged(this, mChecked);
        }
    }

    public void setInitCheck(boolean checked) {
        mChecked = checked;
        reset();
        invalidate();
    }
