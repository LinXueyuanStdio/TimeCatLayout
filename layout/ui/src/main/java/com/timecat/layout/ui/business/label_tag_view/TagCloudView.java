package com.timecat.layout.ui.business.label_tag_view;
 ...
 * @author dlink
 * @date 2018/1/26
 * @description TagCloudView标签云
 * @usage // 1. <com.time.cat.views.label_tag_view.TagCloudView android:id="@+id/tag_cloud_view"
 * android:layout_width="match_parent" android:layout_height="wrap_content"
 * android:background="@drawable/background_tagcloudview_light_selector"
 * app:tcvBackground="@drawable/background_tag_selector" app:tcvBorder="8dp"
 * app:tcvCanTagClick="false" app:tcvItemBorderHorizontal="8dp" app:tcvItemBorderVertical="6dp"
 * app:tcvSingleLine="false" app:tcvTextColor="#42A5F5"/> // 2. tagCloudView.setOnTagClickListener(this);
 * tagCloudView.setOnClickListener(new View.OnClickListener() {
 * @Override public void onActionDown(View view) { Log.e(TAG, "onActionDown tagCloudView"); } }); //
 * 3.
 * <p>
 * //region TagCloudView.OnTagClickListener
 * @Override public void onTagClick(int position) { if (position == -1) { Log.e(TAG, "onActionDown
 * tagCloudView at --> 点击末尾文字"); } else { Log.e(TAG, "onActionDown tagCloudView at --> 点击 position"
 * + position); } } //-//endregion
 */
public class TagCloudView extends ViewGroup {
    ...
    public TagCloudView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mShowRightImage = a
                .getBoolean(R.styleable.TagCloudView_tcvShowRightImg, DEFAULT_SHOW_RIGHT_IMAGE);
            ...
            endText.setOnLongClickListener(new OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    if (onTagLongClickListener != null) {
                        onTagLongClickListener.onTagLongClick(-1);
                        return true;
                    }
                    return false;
                }
    ...
    public void setTags(List<ITag> tagList) {
        this.tags = tagList;
        this.removeAllViews();
        if (tags != null && tags.size() > 0) {
            for (int i = 0; i < tags.size(); i++) {
                TextView tagView = generate(i, tags.get(i));
                addView(tagView);
            }
        } else {
            TextView tagView = generate(-1, new ITag() {
                @Override
    ...
    private TextView generate(int i, ITag tag) {
    ...
    public void removeAllWhereEquals(ITag tag) {
        for (ITag t : tags) {
    ...
    public void removeOneWhereEquals(ITag tag) {
        for (ITag t : tags) {
            if (t.getName().equals(tag.getName())) {
                tags.remove(t);
                break;
    ...
    public void removeAllWhereContains(List<ITag> tagList) {
        for (ITag t : tagList) {
            removeAllWhereEquals(t);
        }
    }
    ...
    public void setOnTagLongClickListener(OnTagLongClickListener onTagClickListener) {
        this.onTagLongClickListener = onTagClickListener;
    }
